...
from form_master.interactors.text_submit_form_interactor \
    import TextQuestionSubmitFormResponseInteractor
...
def test_text_submit_form_interactor_given_invalid_form_raises_exception():
    ...
    interactor = TextQuestionSubmitFormResponseInteractor(
        user_id=user_id, form_id=form_id,
        question_id=question_id, storage=storage,
        user_submitted_text=text
    )
...
def test_text_submit_form_interactor_given_form_status_closed_form_raises_exception(
        form_status_false):
...
def test_text_submit_form_interactor_given_form_status_live_form_raises_exception(
        form_status_true):
...
def test_text_submit_form_interactor_given_invalid_user_response_form_raises_exception(
        form_status_true):
...
def test_text_submit_form_interactor_given_valid_details_returns_response(
        form_status_true, user_text_response_dto):
    ...
    storage.validate_question_id_with_form\
        .return_value = None
    storage.create_user_text_response.return_value = user_text_response_dto
    presenter.raise_invalid_user_response_submitted.side_effect = NotFound
    ...
    storage.create_user_text_response.assert_called_once_with(
        user_response_dto=user_text_response_dto
    )